 ...
 ################## Database view ########################################## 
  ...
  output$ex1<-renderTable({
    ...
    dplyr::slice(Dataset(), 1:input$ntabla)
      ...
      if (input$checkbox=="Ederly")
      { dplyr::slice(Ederly, 1:input$ntabla) }
      else{
        if(input$checkbox=="Neonates")
        {  dplyr::slice(Neonates, 1:input$ntabla)}
      }
    }
  })
...
####################### Variables  #############################################  
  ...
  output$tabla<-renderTable({
    if(length(input$vdetabla)>0){
      ...
      mat2<-select(mat2,cutpoint,sensitivity.50.,specificity.50.)
